...
class Fish3D(Fish2D):
    ...
    def render_o3d_to_image(geoms, width=800, height=600):
    ...
    def measure_curved_length_optimized(self, num_slices=20):
        ...
        # Usamos np.polyval para evaluar los coeficientes de la derivada en los puntos x_fine
        dz_dx = np.polyval(np.polyder(z_poly), x_fine)
        dy_dx = np.polyval(np.polyder(y_poly), x_fine)
